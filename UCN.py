...
def PrintTemperatureVsCycle(ex, pdf):
  ...
  graph.Draw('AP')
  hetemp = lambda x, p: HeTemperature(x[0])
  fHeTemperature = ROOT.TF1('HeTemperature', hetemp, HeTemperature(graph.GetHistogram().GetMinimum()), HeTemperature(graph.GetHistogram().GetMaximum()))
